      ...
      */
      ...
      this.$nextTick(function () {
        ...
        */
         ...
         *
        */
       const delay = vm.$ksvuefp.options.duration ? vm.$ksvuefp.options.duration + vm.$ksvuefp.options.animDelay + 250 : vm.$ksvuefp.options.animDelay + 1250   // EOM changed

        setTimeout(function () {
          vm.$ksvuefp.$emit('ksvuefp-change-done');
        // }, vm.$ksvuefp.options.duration ? vm.$ksvuefp.options.duration + vm.$ksvuefp.options.animDelay + 100 : vm.$ksvuefp.options.animDelay + 1100);        // original
        }, delay);    // EOM changed to improve touchpad scroll performance
      });
    }
  },
  watch: {
  ...
  Vue.prototype.$ksvuefp = new Vue({
    ...
    methods: {
      ...
      canAnimContent: function canAnimContent(index) {
        ...
        if (wait) {
          // return !this.slidingActive ? true : false; // original
          return this.slidingFinished ? true : false; // EOM added
        }
        return true;
      }
    }
